...
rescript-signals is a lightweight, high-performance reactive signals library for [ReScript](https://rescript-lang.org/) with zero dependencies. Build reactive applications with fine-grained updates, with automatic dependency tracking and minimal re-computation, while leveraging the powerful type system from ReScript.
...
rescript-signals focuses on clarity, control, and performance. The goal is to offer precise, fine-grained updates and predictable behavior with a minimal set of abstractions, while leveraging the robust type system from ReScript.
...
Check the [website](https://brnrdog.github.io/rescript-signals/) for comprehensive documentation, API reference, and examples.

## Benchmark Snapshot (CI)

Latest `milomg/js-reactivity-benchmark` report (top frameworks + ReScript Signals), ordered by average runtime per test.
